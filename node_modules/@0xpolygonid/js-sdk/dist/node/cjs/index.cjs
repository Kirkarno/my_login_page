...
var BjjProvider = class {
  ...
  async newPrivateKeyFromSeed(seed) {
    ...
    const privateKey = new import_js_crypto3.PrivateKey(seed);
  ...
  async sign(keyId, data) {
    ...
    const i = import_js_iden3_core2.BytesHelper.bytesToInt(data);
    if (!(0, import_js_iden3_core2.checkBigIntInField)(i)) {
      throw new Error("data to sign is too large");
    }
    const privateKey = await this.privateKey(keyId);
  ...
  async privateKey(keyId) {
    ...
    return new import_js_crypto3.PrivateKey(import_js_crypto3.Hex.decodeString(privateKeyHex));
  }
  async verify(message, signatureHex, keyId) {
    const publicKey = await this.publicKey(keyId);
    return import_js_crypto3.PublicKey.newFromCompressed(hexToBytes(publicKey)).verifyPoseidon(
      import_js_iden3_core2.BytesHelper.bytesToInt(message),
      import_js_crypto3.Signature.newFromCompressed(hexToBytes(signatureHex))
    );
  }
};
...
var Ed25519Provider = class {
  ...
  async newPrivateKeyFromSeed(seed) {
    ...
    const publicKey = import_ed25519.ed25519.getPublicKey(seed);
    const kmsId = {
      type: this.keyType,
  ...
  async publicKey(keyId) {
    ...
    const publicKey = import_ed25519.ed25519.getPublicKey(privateKeyHex);
  ...
  async sign(keyId, digest) {
    const privateKeyHex = await this.privateKey(keyId);
    return import_ed25519.ed25519.sign(digest, privateKeyHex);
  }
  /**
  ...
  async verify(digest, signatureHex, keyId) {
    ...
    return import_ed25519.ed25519.verify(signatureHex, digest, publicKeyHex);
  }
  /**
...
var Sec256k1Provider = class {
  ...
  async publicKey(keyId) {
    const privateKeyHex = await this.privateKey(keyId);
    const publicKey = import_secp256k1.secp256k1.getPublicKey(privateKeyHex, false);
    return bytesToHex(publicKey);
  }
  /**
  ...
  async verify(message, signatureHex, keyId) {
    const publicKeyHex = await this.publicKey(keyId);
    return import_secp256k1.secp256k1.verify(signatureHex, (0, import_js_crypto4.sha256)(message), publicKeyHex);
  }
  async privateKey(keyId) {
    return this._keyStore.get({ alias: keyId.id });
  }
...
var Iden3SparseMerkleTreeProof = class _Iden3SparseMerkleTreeProof {
  ...
  static fromJSON(obj) {
    ...
    return new _Iden3SparseMerkleTreeProof({
      coreClaim: new import_js_iden3_core3.Claim().fromHex(obj.coreClaim),
      mtp,
      issuerData: {
        id: import_js_iden3_core3.DID.parse(obj.issuerData.id),
        state: {
...
var BJJSignatureProof2021 = class _BJJSignatureProof2021 {
  ...
  toJSON() {
    return {
      ...
      type: this.type,
      coreClaim: this.coreClaim.hex(),
      signature: import_js_crypto5.Hex.encodeString(this.signature.compress())
    };
  }
  /**
  ...
  static fromJSON(obj) {
    return new _BJJSignatureProof2021({
      issuerData: {
        ...
        authCoreClaim: new import_js_iden3_core3.Claim().fromHex(obj.issuerData.authCoreClaim)
      },
      coreClaim: new import_js_iden3_core3.Claim().fromHex(obj.coreClaim),
      signature: import_js_crypto5.Signature.newFromCompressed(
        Uint8Array.from(import_js_crypto5.Hex.decodeString(obj.signature)).slice(0, 64)
      )
    });
  }
};
...
var getUserDIDFromCredential = (issuerDID, credential) => {
  ...
  return import_js_iden3_core4.DID.parse(credential.credentialSubject.id);
};
var getKMSIdByAuthCredential = (credential) => {
  ...
  const pb = new import_js_crypto6.PublicKey([BigInt(x), BigInt(y)]);
...
var fillCoreClaimSlot = async (slotData, mz, path) => {
  ...
  try {
    ...
    const bytesVal = import_js_iden3_core5.BytesHelper.intToBytes(intVal);
...
var calculateCoreSchemaHash = (schemaId) => {
  ...
  return new import_js_iden3_core5.SchemaHash(sHash.slice(sHash.length - 16, sHash.length));
};
...
var W3CCredential = class _W3CCredential {
  ...
  static buildCredentialStatus(request, issuer) {
    ...
    switch (request.revocationOpts.type) {
      ...
      case "Iden3OnchainSparseMerkleTreeProof2023" /* Iden3OnchainSparseMerkleTreeProof2023 */: {
        const issuerId = import_js_iden3_core6.DID.idFromDID(issuer);
        const chainId = (0, import_js_iden3_core6.getChainId)(import_js_iden3_core6.DID.blockchainFromId(issuerId), import_js_iden3_core6.DID.networkIdFromId(issuerId));
        const searchParams = [
  ...
  async toCoreClaim(opts) {
    ...
    const claim = import_js_iden3_core6.Claim.newClaim(
      schemaHash,
      import_js_iden3_core6.ClaimOptions.withIndexDataBytes(slots.indexA, slots.indexB),
      import_js_iden3_core6.ClaimOptions.withValueDataBytes(slots.valueA, slots.valueB),
      import_js_iden3_core6.ClaimOptions.withRevocationNonce(BigInt(opts.revNonce)),
      import_js_iden3_core6.ClaimOptions.withVersion(opts.version)
    );
    if (opts.updatable) {
    ...
    if (subjectId) {
      const did = import_js_iden3_core6.DID.parse(subjectId.toString());
      const id = import_js_iden3_core6.DID.idFromDID(did);
  ...
  async verifyCoreClaimMatch(coreClaim, merklizeOpts) {
    ...
    switch (merklizedPosition) {
      case import_js_iden3_core6.MerklizedRootPosition.None:
        merklizedRootPosition = "" /* None */;
        break;
      case import_js_iden3_core6.MerklizedRootPosition.Index:
        merklizedRootPosition = "index" /* Index */;
        break;
      case import_js_iden3_core6.MerklizedRootPosition.Value:
    ...
    switch (idPosition) {
      case import_js_iden3_core6.IdPosition.None:
        subjectPosition = "" /* None */;
        break;
      case import_js_iden3_core6.IdPosition.Index:
        subjectPosition = "index" /* Index */;
        break;
      case import_js_iden3_core6.IdPosition.Value:
  ...
  async verifyBJJSignatureProof(proof, coreClaim, resolverURL, userDID, credStatusResolverRegistry) {
    ...
    const pubKey = new import_js_crypto7.PublicKey([rawSlotsInt[2], rawSlotsInt[3]]);
    const { hi, hv } = coreClaim.hiHv();
    const claimHash = import_js_crypto7.poseidon.hash([hi, hv]);
...
function extractProof(proof) {
  ...
  if (typeof proof === "object") {
    ...
    const coreClaim = p.coreClaim instanceof import_js_iden3_core6.Claim ? p.coreClaim : new import_js_iden3_core6.Claim().fromHex(p.coreClaim);
...
function validateTreeState(treeState) {
  ...
  const wantState = import_js_crypto7.poseidon.hash([ctrHash.bigInt(), rtrHash.bigInt(), rorHash.bigInt()]);
...
var subjectPositionIndex = (idPosition) => {
  switch (idPosition) {
    case import_js_iden3_core7.IdPosition.Index:
      return "index" /* Index */;
    case import_js_iden3_core7.IdPosition.Value:
      return "value" /* Value */;
    default:
      return "" /* None */;
  }
...
var defineMerklizedRootPosition = (metadata, position) => {
  if (!metadata?.serialization) {
    return import_js_iden3_core7.MerklizedRootPosition.None;
  }
  if (position != null && position !== import_js_iden3_core7.MerklizedRootPosition.None) {
    return position;
  }
  return import_js_iden3_core7.MerklizedRootPosition.Index;
};
var generateProfileDID = (did, profileNonce) => {
  const id = import_js_iden3_core7.DID.idFromDID(did);
  profileNonce = profileNonce ?? 0;
  if (!isBigInt(profileNonce)) {
    throw new Error("profile must be number or decimal string");
  }
  const profile = import_js_iden3_core7.Id.profileId(id, BigInt(profileNonce));
  return import_js_iden3_core7.DID.parseFromId(profile);
};
var isBigInt = (x) => {
  try {
...
function fieldToByteArray(field) {
  ...
  return import_js_iden3_core8.BytesHelper.intToBytes(bigIntField);
}
function dataFillsSlot(slot, newData) {
  return (0, import_js_iden3_core8.checkBigIntInField)(import_js_iden3_core8.BytesHelper.bytesToInt(Uint8Array.from([...slot, ...newData])));
}
function checkDataInField(data) {
  return (0, import_js_iden3_core8.checkBigIntInField)(import_js_iden3_core8.BytesHelper.bytesToInt(data));
}
...
var AtomicQueryMTPV2PubSignals = class extends BaseConfig {
  ...
  pubSignalsUnmarshal(data) {
    ...
    fieldIdx++;
    this.userID = import_js_iden3_core9.Id.fromBigInt(BigInt(sVals[fieldIdx]));
    fieldIdx++;
    this.requestID = BigInt(sVals[fieldIdx]);
    fieldIdx++;
    this.issuerID = import_js_iden3_core9.Id.fromBigInt(BigInt(sVals[fieldIdx]));
    fieldIdx++;
    ...
    fieldIdx++;
    this.claimSchema = import_js_iden3_core9.SchemaHash.newSchemaHashFromInt(BigInt(sVals[fieldIdx]));
    fieldIdx++;
...
var AtomicQueryMTPV2OnChainPubSignals = class extends BaseConfig {
  ...
  pubSignalsUnmarshal(data) {
    ...
    this.merklized = parseInt(sVals[fieldIdx]);
    fieldIdx++;
    this.userID = import_js_iden3_core10.Id.fromBigInt(BigInt(sVals[fieldIdx]));
    fieldIdx++;
    ...
    fieldIdx++;
    this.issuerID = import_js_iden3_core10.Id.fromBigInt(BigInt(sVals[fieldIdx]));
    fieldIdx++;
...
var AtomicQueryV3Inputs = class extends BaseConfig {
  ...
  fillSigProofWithZero(s) {
    ...
    s.issuerAuthClaim = new import_js_iden3_core12.Claim().marshalJson();
...
var AtomicQueryV3PubSignals = class extends BaseConfig {
  ...
  pubSignalsUnmarshal(data) {
    ...
    fieldIdx++;
    this.issuerState = import_js_merkletree9.Hash.fromString(sVals[fieldIdx]);
    fieldIdx++;
    ...
    this.isRevocationChecked = parseInt(sVals[fieldIdx]);
    fieldIdx++;
    this.issuerClaimNonRevState = import_js_merkletree9.Hash.fromString(sVals[fieldIdx]);
    fieldIdx++;
    this.timestamp = parseInt(sVals[fieldIdx]);
    fieldIdx++;
    this.claimSchema = import_js_iden3_core12.SchemaHash.newSchemaHashFromInt(BigInt(sVals[fieldIdx]));
    fieldIdx++;
    this.claimPathKey = BigInt(sVals[fieldIdx]);
    fieldIdx++;
    this.slotIndex = parseInt(sVals[fieldIdx]);
    ...
    fieldIdx++;
    if (sVals[fieldIdx] !== "0") {
      this.verifierID = import_js_iden3_core12.Id.fromBigInt(BigInt(sVals[fieldIdx]));
    }
    fieldIdx++;
...
var AtomicQueryV3OnChainInputs = class extends BaseConfig {
  ...
  fillSigProofWithZero(s) {
    ...
    s.issuerAuthClaim = new import_js_iden3_core13.Claim().marshalJson();
  ...
  fillAuthWithZero(s) {
    s.authClaim = new import_js_iden3_core13.Claim().marshalJson();
...
var AtomicQuerySigV2OnChainPubSignals = class extends BaseConfig {
  ...
  pubSignalsUnmarshal(data) {
    ...
    this.merklized = parseInt(sVals[fieldIdx]);
    fieldIdx++;
    this.userID = import_js_iden3_core14.Id.fromBigInt(BigInt(sVals[fieldIdx]));
    fieldIdx++;
    this.circuitQueryHash = BigInt(sVals[fieldIdx]);
    fieldIdx++;
    ...
    fieldIdx++;
    this.issuerID = import_js_iden3_core14.Id.fromBigInt(BigInt(sVals[fieldIdx]));
    fieldIdx++;
    this.isRevocationChecked = parseInt(sVals[fieldIdx]);
    fieldIdx++;
...
var AuthV2PubSignals = class {
  ...
  pubSignalsUnmarshal(data) {
    ...
    this.userID = import_js_iden3_core15.Id.fromBigInt(BigInt(sVals[0]));
...
var StateTransitionPubSignals = class {
  ...
  pubSignalsUnmarshal(data) {
    ...
    this.userId = import_js_iden3_core16.Id.fromBigInt(BigInt(sVals[0]));
...
var packZkpProof = (inputs, a, b, c) => {
  return new import_ethers3.ethers.AbiCoder().encode(
...
function createInMemoryCache(params) {
  const cache = new import_quick_lru.default({ maxSize: params.maxSize, maxAge: params.ttl });
  return {
    get: async (key) => {
...
var PackageManager = class {
  ...
  getMediaType(envelope) {
    ...
    if (envelope[0] === "{") {
      ...
      base64HeaderBytes = import_rfc46482.base64.parse(header2, { loose: true });
    }
...
var extractPublicKeyBytes = (vm) => {
  ...
  if (vm.publicKeyJwk && vm.publicKeyJwk.crv === "secp256k1" && vm.publicKeyJwk.x && vm.publicKeyJwk.y) {
    ...
    return {
      publicKeyBytes: import_secp256k12.secp256k1.ProjectivePoint.fromAffine({
        x,
        y
      }).toRawBytes(false),
      kmsKeyType: "Secp256k1" /* Secp256k1 */
...
// src/iden3comm/packers/zkp.ts
var { getProvingMethod } = import_js_jwz2.proving;
...
var ZKPPacker = class {
  ...
  async pack(payload, params) {
    ...
    const token = new import_js_jwz2.Token(
      provingMethod,
    ...
    token.setHeader(import_js_jwz2.Header.Type, "application/iden3-zkp-json" /* ZKPMessage */);
   ...
   */
  async unpack(envelope) {
    const token = await import_js_jwz2.Token.parse(byteDecoder.decode(envelope));
    const provingMethodAlg = new import_js_jwz2.ProvingMethodAlg(token.alg, token.circuitId);
    ...
    case "authV2" /* AuthV2 */:
      {
        ...
        const did = import_js_iden3_core17.DID.parseFromId(authSignals.userID);
        const msgHash = await token.getMessageHash();
        const challenge = import_js_iden3_core17.BytesHelper.bytesToInt(msgHash.reverse());
...
var calcChallengeAuthV2 = (senderAddress, zkpResponses) => {
  ...
  return BigInt(
    import_ethers4.ethers.keccak256(
      new import_ethers4.ethers.AbiCoder().encode(
...
var packMetadatas = (metas) => {
  return new import_ethers4.ethers.AbiCoder().encode(["tuple(string key,bytes value)[]"], [metas]);
};
var verifyExpiresTime = (message) => {
  if (message?.expires_time && message.expires_time < (0, import_js_iden3_core18.getUnixTimestamp)(/* @__PURE__ */ new Date())) {
...
var MessageHandler = class {
  ...
  async handleMessage(bytes, context) {
    ...
    if (unpackedMediaType === "application/iden3-zkp-json" /* ZKPMessage */ && senderDid) {
      packerParams = {
        senderDID: senderDid,
        provingMethodAlg: import_js_jwz3.proving.provingMethodGroth16AuthV2Instance.methodAlg
      };
...
function createAuthorizationRequestWithMessage(reason, message, sender, callbackUrl, opts) {
  ...
  const request = {
    ...
    created_time: (0, import_js_iden3_core19.getUnixTimestamp)(/* @__PURE__ */ new Date()),
    expires_time: opts?.expires_time ? (0, import_js_iden3_core19.getUnixTimestamp)(opts.expires_time) : void 0
  };
  return request;
}
var AuthHandler = class extends AbstractMessageHandler {
  ...
  async handleAuthRequest(authRequest, ctx) {
    ...
    const to = authRequest.to ? import_js_iden3_core19.DID.parse(authRequest.to) : ctx.senderDid;
    const guid = uuid2.v4();
    if (!authRequest.from) {
    ...
    const from = import_js_iden3_core19.DID.parse(authRequest.from);
  ...
  async handleAuthorizationRequest(did, request, opts) {
    ...
    const packerOpts = opts.mediaType === "application/iden3comm-signed-json" /* SignedMessage */ ? opts.packerOptions : {
      provingMethodAlg: import_js_jwz4.proving.provingMethodGroth16AuthV2Instance.methodAlg
    };
  ...
  async handleAuthResponse(response, ctx) {
    ...
    for (const proofRequest of requestScope) {
      ...
      params.verifierDid = import_js_iden3_core19.DID.parse(request.from);
...
var OnChainRevocationStorage = class _OnChainRevocationStorage {
  ...
  constructor(_config, contractAddress, _signer) {
    ...
    this._provider = new import_ethers5.JsonRpcProvider(_config.url);
    let contract = new import_ethers5.Contract(contractAddress, CredentialStatusResolver_default, this._provider);
    if (this._signer) {
...
var OnChainResolver = class {
  ...
  async getRevocationOnChain(credentialStatus, issuer) {
    ...
    const issuerId = import_js_iden3_core20.DID.idFromDID(issuer);
    ...
    const id = import_js_iden3_core20.DID.idFromDID(issuer);
...
var RHSResolver = class {
  ...
  async getStatus(credentialStatus, issuerDID, issuerData, genesisState) {
    const issuerId = import_js_iden3_core21.DID.idFromDID(issuerDID);
    let latestState;
    try {
...
function isIssuerGenesis(issuer, state) {
  const did = import_js_iden3_core21.DID.parse(issuer);
  const id = import_js_iden3_core21.DID.idFromDID(did);
  const { method, blockchain, networkId } = import_js_iden3_core21.DID.decodePartsFromId(id);
  const arr = import_js_iden3_core21.BytesHelper.hexToBytes(state);
  const stateBigInt = import_js_iden3_core21.BytesHelper.bytesToInt(arr);
  const type = (0, import_js_iden3_core21.buildDIDType)(method, blockchain, networkId);
  return isGenesisStateId(import_js_iden3_core21.DID.idFromDID(did).bigInt(), stateBigInt, type);
}
function isGenesisStateId(id, state, type) {
  const idFromState = import_js_iden3_core21.Id.idGenesisFromIdenState(type, state);
...
var CredentialWallet = class {
  ...
  async getRevocationStatusFromCredential(cred) {
    ...
    const issuerDID = import_js_iden3_core22.DID.parse(cred.issuer);
...
var FetchHandler = class _FetchHandler extends AbstractMessageHandler {
  ...
  async handleOnchainOfferMessage(offerMessage) {
    ...
    for (const credentialInfo of offerMessage.body.credentials) {
      const issuerDID = import_js_iden3_core23.DID.parse(offerMessage.from);
      const userDID = import_js_iden3_core23.DID.parse(offerMessage.to);
  ...
  async handleOfferMessage(offerMessage, ctx) {
    ...
    for (const credentialInfo of offerMessage.body.credentials) {
      ...
      const packerOpts = ctx.mediaType === "application/iden3comm-signed-json" /* SignedMessage */ ? ctx.packerOptions : {
        provingMethodAlg: import_js_jwz5.proving.provingMethodGroth16AuthV2Instance.methodAlg
      };
      const senderDID = import_js_iden3_core23.DID.parse(offerMessage.to);
      const token = byteDecoder.decode(
  ...
  async handleFetchRequest(msgRequest) {
    ...
    const issuerDID = import_js_iden3_core23.DID.parse(msgRequest.to);
    const userDID = import_js_iden3_core23.DID.parse(msgRequest.from);
...
var ContractRequestHandler = class extends AbstractMessageHandler {
  ...
  async handleContractInvoke(message, ctx) {
    ...
    const networkFlag = Object.keys(import_js_iden3_core24.ChainIds).find((key) => import_js_iden3_core24.ChainIds[key] === chain_id);
    if (!networkFlag) {
      throw new Error(`Invalid chain id ${chain_id}`);
    }
    const verifierDid = message.from ? import_js_iden3_core24.DID.parse(message.from) : void 0;
    ...
    const zkpResponses = await processZeroKnowledgeProofRequests(
      ...
      {
        ethSigner,
        challenge: challenge ?? import_js_iden3_core24.BytesHelper.bytesToInt(hexToBytes(await ethSigner.getAddress())),
    ...
    switch (methodId) {
      ...
      case "06c86a91" /* SubmitResponse */: {
        ...
        const identifier = import_js_iden3_core24.DID.parse(message.to);
  ...
  async createContractInvokeResponse(request, txHashToZkpResponseMap) {
    const contractInvokeResponse = {
      ...
      created_time: (0, import_js_iden3_core24.getUnixTimestamp)(/* @__PURE__ */ new Date())
    };
  ...
  async handleContractInvokeRequest(did, request, opts) {
    ...
    const verifierDid = ciRequest.from ? import_js_iden3_core24.DID.parse(ciRequest.from) : void 0;
    const zkpResponses = await processZeroKnowledgeProofRequests(
      did,
...
var import_js_jwz6 = require("@iden3/js-jwz");
...
var RefreshHandler = class {
  ...
  async refreshCredential(credential, opts) {
    ...
    const senderDID = import_js_iden3_core25.DID.parse(otherIdentifier);
    const zkpParams = {
      senderDID,
      provingMethodAlg: new import_js_jwz6.ProvingMethodAlg(
        import_js_jwz6.proving.provingMethodGroth16AuthV2Instance.methodAlg.alg,
        "authV2" /* AuthV2 */
      )
    };
    const refreshMsg = {
...
var import_js_jwz7 = require("@iden3/js-jwz");
var RevocationStatusHandler = class extends AbstractMessageHandler {
  ...
  async handleRevocationStatusRequestMessage(rsRequest, context) {
    ...
    const issuerDID = import_js_iden3_core26.DID.parse(rsRequest.to);
  ...
  async handleRevocationStatusRequest(did, request, opts) {
    ...
    const packerOpts = opts.mediaType === "application/iden3comm-signed-json" /* SignedMessage */ ? opts.packerOptions : {
      provingMethodAlg: import_js_jwz7.proving.provingMethodGroth16AuthV2Instance.methodAlg
    };
    if (!rsRequest.to) {
      throw new Error(`failed request. empty 'to' field`);
    }
    const senderDID = import_js_iden3_core26.DID.parse(rsRequest.to);
...
var import_js_jwz8 = require("@iden3/js-jwz");
function createProposalRequest(sender, receiver, opts) {
  ...
  const request = {
    ...
    expires_time: opts?.expires_time ? (0, import_js_iden3_core27.getUnixTimestamp)(opts.expires_time) : void 0
  };
  return request;
}
...
var CredentialProposalHandler = class extends AbstractMessageHandler {
  ...
  async handleProposalRequest(request, opts) {
    ...
    const senderDID = import_js_iden3_core27.DID.parse(proposalRequest.from);
    ...
    const packerOpts = this._params.packerParams.mediaType === "application/iden3comm-signed-json" /* SignedMessage */ ? this._params.packerParams.packerOptions : {
      provingMethodAlg: import_js_jwz8.proving.provingMethodGroth16AuthV2Instance.methodAlg
    };
...
var import_js_jwz9 = require("@iden3/js-jwz");
...
function createPaymentRequest(sender, receiver, agent, payments, opts) {
  ...
  const request = {
    ...
    created_time: (0, import_js_iden3_core28.getUnixTimestamp)(/* @__PURE__ */ new Date()),
    expires_time: opts?.expires_time ? (0, import_js_iden3_core28.getUnixTimestamp)(opts.expires_time) : void 0
  };
  return request;
}
async function verifyEIP712TypedData(data, resolver) {
  const paymentData = data.type === "Iden3PaymentRailsRequestV1" /* Iden3PaymentRailsRequestV1 */ ? {
    ...
    amount: data.amount,
    expirationDate: (0, import_js_iden3_core28.getUnixTimestamp)(new Date(data.expirationDate)),
    nonce: data.nonce,
    metadata: "0x"
  } : {
  ...
  const recovered = import_ethers6.ethers.verifyTypedData(
    proof.eip712.domain,
    types,
    paymentData,
    proof.proofValue
...
var PaymentHandler = class extends AbstractMessageHandler {
  ...
  async handlePaymentRequestMessage(paymentRequest, ctx) {
    ...
    const senderDID = import_js_iden3_core28.DID.parse(paymentRequest.to);
    const receiverDID = import_js_iden3_core28.DID.parse(paymentRequest.from);
    const payments = [];
    ...
    for (let i = 0; i < payments.length; i++) {
      ...
      for (let j = 0; j < payments[i].options.length; j++) {
        ...
        const paymentData = option.type === "Iden3PaymentRailsRequestV1" /* Iden3PaymentRailsRequestV1 */ ? {
          recipient,
          amount,
          expirationDate: (0, import_js_iden3_core28.getUnixTimestamp)(expirationDateRequired),
          nonce,
          metadata: "0x"
        } : {
  ...
  async packMessage(message, senderDID) {
    ...
    const packerOpts = this._params.packerParams.mediaType === "application/iden3comm-signed-json" /* SignedMessage */ ? this._params.packerParams.packerOptions : {
      provingMethodAlg: import_js_jwz9.proving.provingMethodGroth16AuthV2Instance.methodAlg
    };
...
function createDiscoveryFeatureQueryMessage(queries, opts) {
  ...
  return {
    ...
    from: opts?.from,
    to: opts?.to,
    created_time: (0, import_js_iden3_core29.getUnixTimestamp)(/* @__PURE__ */ new Date()),
  ...
  async handleDiscoveryQuery(message, opts) {
    ...
    return Promise.resolve(
      createDiscoveryFeatureDiscloseMessage(disclosures, {
        to: message.from,
        from: message.to,
        expires_time: opts?.disclosureExpiresDate ? (0, import_js_iden3_core29.getUnixTimestamp)(opts.disclosureExpiresDate) : void 0
      })
    );
  }
  handleQuery(query) {
...
var EthStateStorage = class {
  ...
  constructor(ethConfig, options) {
    ...
    this.provider = new import_ethers7.JsonRpcProvider(config.url);
    this.stateContract = new import_ethers7.Contract(config.contractAddress, State_default, this.provider);
  ...
  getStateContractAndProviderForId(id) {
    const idTyped = import_js_iden3_core30.Id.fromBigInt(id);
    const chainId = (0, import_js_iden3_core30.getChainId)(import_js_iden3_core30.DID.blockchainFromId(idTyped), import_js_iden3_core30.DID.networkIdFromId(idTyped));
    const config = this.networkByChainId(chainId);
    const provider = new import_ethers7.JsonRpcProvider(config.url);
    const stateContract = new import_ethers7.Contract(config.contractAddress, State_default, provider);
...
var OnChainZKPVerifier = class _OnChainZKPVerifier {
  ...
  async submitZKPResponse(ethSigner, txData, zkProofResponses) {
    ...
    const provider = new import_ethers8.JsonRpcProvider(chainConfig.url, chainConfig.chainId);
    ...
    const verifierContract = new import_ethers8.Contract(txData.contract_address, ZkpVerifier_default);
  ...
  async submitResponse(ethSigner, txData, responses, authProof) {
    ...
    const maxFeePerGas = chainConfig.maxFeePerGas ? BigInt(chainConfig.maxFeePerGas) : feeData.maxFeePerGas;
    const maxPriorityFeePerGas = chainConfig.maxPriorityFeePerGas ? BigInt(chainConfig.maxPriorityFeePerGas) : feeData.maxPriorityFeePerGas;
    const verifierContract = new import_ethers8.Contract(txData.contract_address, import_universal_verifier_v2_abi.IVerifierABI);
  ...
  static getCrossChainResolvers(source, txDataChainId, type, didResolverUrl) {
    return [
      ...
      const id = import_js_iden3_core31.Id.fromString(info.id);
      const chainId = (0, import_js_iden3_core31.chainIDfromDID)(import_js_iden3_core31.DID.parseFromId(id));
      if (txDataChainId === chainId) {
        return acc;
      }
      const promise = this.resolveDidDocumentEip712MessageAndSignature(
        import_js_iden3_core31.DID.parseFromId(import_js_iden3_core31.Id.fromString(info.id)),
        didResolverUrl,
        {
  ...
  static packCrossChainProofs(gistUpdateArr, stateUpdateArr) {
    ...
    return new import_ethers8.ethers.AbiCoder().encode(
...
var packEthIdentityProof = (did) => {
  return `0x${bytesToHex(import_js_iden3_core31.BytesHelper.intToBytes(import_js_iden3_core31.DID.idFromDID(did).bigInt()))}`;
};
...
var OnchainNonMerklizedIssuerAdapter = class {
  ...
  constructor(ethConnectionConfig, issuerDid, options) {
    ...
    this._contractAddress = import_ethers9.ethers.getAddress(
      import_ethers9.ethers.hexlify(import_js_iden3_core32.Id.ethAddressFromId(import_js_iden3_core32.DID.idFromDID(issuerDid)))
    );
    this._contract = import_onchain_non_merklized_issuer_base_abi.NonMerklizedIssuerBase__factory.connect(
      this._contractAddress,
      new import_ethers9.ethers.JsonRpcProvider(ethConnectionConfig.url)
    );
  ...
  async convertOnChainInfoToW3CCredential(credentialData, coreClaimBigInts, credentialSubjectFields) {
    const c = new import_js_iden3_core32.Claim().unMarshalJson(JSON.stringify(coreClaimBigInts.map((b) => b.toString())));
    ...
    const credentialRequest = {
      ...
      issuanceDate: (0, import_js_iden3_core33.getDateFromUnixTimestamp)(Number(credentialData.issuanceDate)).getTime()
    };
  ...
  async convertCredentialSubject(coreClaim, contractContexts, credentialType, credentialSubjectFields) {
    ...
    for (const f of credentialSubjectFields) {
      ...
      switch (dataType) {
        ...
        case "http://www.w3.org/2001/XMLSchema#dateTime" /* DateTime */: {
          ...
          const sourceTimestamp = (0, import_js_iden3_core33.getDateFromUnixTimestamp)(Number(timestamp)).toISOString();
    ...
    const subjectDid = import_js_iden3_core32.DID.parseFromId(subjectId);
...
var OnchainIssuer = class {
  ...
  async getCredential(issuerDID, userDID, credentialId) {
    ...
    switch (response) {
      case "0.0.1": {
        ...
        const { credentialData, coreClaimBigInts, credentialSubjectFields } = await adapter.getCredential(import_js_iden3_core34.DID.idFromDID(userDID), credentialId);
  ...
  async getUserCredentialIds(issuerDID, userDID) {
    ...
    switch (response) {
      case "0.0.1": {
        ...
        await adapter.isInterfaceSupported();
        return await adapter.getUserCredentialsIds(import_js_iden3_core34.DID.idFromDID(userDID));
      }
      default:
        throw new Error(`Unsupported adapter version ${response}`);
    }
  ...
  getContractConnection(did) {
    const issuerId = import_js_iden3_core34.DID.idFromDID(did);
    const chainId = (0, import_js_iden3_core34.chainIDfromDID)(did);
    const contractAddress = import_ethers10.ethers.getAddress(import_ethers10.ethers.hexlify(import_js_iden3_core34.Id.ethAddressFromId(issuerId)));
    ...
    const contract = new import_ethers10.Contract(contractAddress, import_onchain_non_merklized_issuer_base_abi2.INonMerklizedIssuerABI, new import_ethers10.ethers.JsonRpcProvider(connection.url));
...
var DidResolverStateReadonlyStorage = class {
  ...
  async getGISTProof(id) {
    const { didDocument } = await resolveDidDocument(
      import_js_iden3_core35.DID.parseFromId(import_js_iden3_core35.Id.fromBigInt(id)),
      this.resolverUrl
    );
  ...
  async getGISTRootInfo(root, userId) {
    const { didDocument } = await resolveDidDocument(
      import_js_iden3_core35.DID.parseFromId(import_js_iden3_core35.Id.fromBigInt(userId)),
      this.resolverUrl,
      {
  ...
  getRpcProvider() {
    return new import_ethers11.JsonRpcProvider();
  }
  publishState() {
...
async function getPermitSignature(signer, tokenAddress, spender, value, deadline) {
  const erc20PermitContract = new import_ethers12.Contract(tokenAddress, ERC20Permit_default, signer);
...
async function getERC20Decimals(tokenAddress, runner) {
  const erc20Contract = new import_ethers12.Contract(tokenAddress, ERC20_default, runner);
...
var import_js_jwz10 = require("@iden3/js-jwz");
...
var NativeProver = class _NativeProver {
  ...
  async verify(zkp, circuitId) {
    try {
      ...
      return (0, import_js_jwz10.verifyGroth16Proof)(zkp, JSON.parse(byteDecoder.decode(circuitData.verificationKey)));
    } catch (e) {
      console.log(e);
      return false;
    }
...
var InputGenerator = class {
  ...
  async newCircuitClaimData(preparedCredential) {
    ...
    circuitClaim.issuerId = import_js_iden3_core36.DID.idFromDID(import_js_iden3_core36.DID.parse(preparedCredential.credential.issuer));
    if (smtProof) {
  ...
  credentialAtomicQueryMTPV2PrepareInputs = async ({
    ...
    circuitInputs.id = import_js_iden3_core36.DID.idFromDID(identifier);
    ...
    circuitInputs.currentTimeStamp = (0, import_js_iden3_core36.getUnixTimestamp)(/* @__PURE__ */ new Date());
  ...
  credentialAtomicQueryMTPV2OnChainPrepareInputs = async ({
    ...
    const id = import_js_iden3_core36.DID.idFromDID(identifier);
    circuitInputs.id = import_js_iden3_core36.DID.idFromDID(identifier);
    circuitInputs.requestID = BigInt(proofReq.id);
  ...
  credentialAtomicQueryV3PrepareInputs = async ({
    ...
    circuitInputs.verifierID = params.verifierDid ? import_js_iden3_core36.DID.idFromDID(params.verifierDid) : void 0;
...
async function checkQueryRequest(query, queriesMetadata, ldContext, outputs, circuitId, schemaLoader, opts) {
  const userDID = import_js_iden3_core37.DID.parseFromId(outputs.issuerId);
  ...
  const timeDiff = Date.now() - (0, import_js_iden3_core37.getDateFromUnixTimestamp)(Number(outputs.timestamp)).getTime();
...
function calculateGroupId(requestIds) {
  ...
  const groupID = BigInt(import_ethers13.ethers.keccak256(import_ethers13.ethers.solidityPacked(types, requestIds))) & // It should fit in a field number in the circuit (max 253 bits). With this we truncate to 252 bits for the group ID
  BigInt("0x0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
  return groupID;
}
function calculateRequestId(requestParams, creatorAddress) {
  const requestId = (BigInt(
    import_ethers13.ethers.keccak256(import_ethers13.ethers.solidityPacked(["bytes", "address"], [requestParams, creatorAddress]))
...
function calculateMultiRequestId(requestIds, groupIds, creatorAddress) {
  return BigInt(
    import_ethers13.ethers.keccak256(
      import_ethers13.ethers.solidityPacked(
...
var PubSignalsVerifier = class {
  ...
  credentialAtomicQueryV3Verify = async ({
    ...
    if (nSessionId !== 0n) {
      ...
      const id = import_js_iden3_core38.DID.idFromDID(verifierDIDParam);
  ...
  authV2Verify = async ({
    ...
    if (!gist.latest) {
      const timeDiff = Date.now() - (0, import_js_iden3_core38.getDateFromUnixTimestamp)(Number(gist.transitionTimestamp)).getTime();
  ...
  verifyIdOwnership = (sender, challenge) => {
    const senderId = import_js_iden3_core38.DID.idFromDID(import_js_iden3_core38.DID.parse(sender));
  ...
  async resolve(id, state) {
    ...
    const did = import_js_iden3_core38.DID.parseFromId(id);
  ...
  checkRevocationState = async (issuerID, issuerClaimNonRevState, opts) => {
    ...
    if (!issuerNonRevStateResolved.latest) {
      const timeDiff = Date.now() - (0, import_js_iden3_core38.getDateFromUnixTimestamp)(Number(issuerNonRevStateResolved.transitionTimestamp)).getTime();
      if (timeDiff > acceptedStateTransitionDelay) {
...
var ProofService = class {
  ...
  async generateProof(proofReq, identifier, opts) {
    ...
    const subjectDID = import_js_iden3_core39.DID.parse(preparedCredential.credential.credentialSubject["id"]);
    ...
    let mk;
    if (mtPosition !== import_js_iden3_core39.MerklizedRootPosition.None) {
  ...
  async generateAuthProof(circuitId, identifier, opts) {
    ...
    switch (circuitId) {
      case "authV2" /* AuthV2 */:
        {
          const challenge = opts.challenge ? import_js_iden3_core39.BytesHelper.intToBytes(opts.challenge).reverse() : new Uint8Array(32);
  ...
  async generateAuthV2Inputs(hash, did, circuitId) {
    ...
    const challenge = import_js_iden3_core39.BytesHelper.bytesToInt(hash.reverse());
  ...
  async verifyState(circuitId, pubSignals, opts = {
    ...
    if (globalStateInfo.replacedByRoot !== 0n) {
      ...
      const timeDiff = Date.now() - (0, import_js_iden3_core39.getDateFromUnixTimestamp)(Number(globalStateInfo.replacedAtTimestamp)).getTime();
...
var IdentityWallet = class {
  ...
  async createAuthCoreClaim(revNonce, seed) {
    ...
    const schemaHash = import_js_iden3_core40.SchemaHash.authSchemaHash;
    const authClaim = import_js_iden3_core40.Claim.newClaim(
      schemaHash,
      import_js_iden3_core40.ClaimOptions.withIndexDataInts(pubKey.p[0], pubKey.p[1]),
      import_js_iden3_core40.ClaimOptions.withRevocationNonce(BigInt(0))
    );
  ...
  async createAuthBJJCredential(did, pubKey, authClaim, currentState, revocationOpts) {
    ...
    const expiration = authData ? (0, import_js_iden3_core40.getUnixTimestamp)(authData) : 0;
    const request = {
  ...
  async createIdentity(opts) {
    ...
    const didType = (0, import_js_iden3_core40.buildDIDType)(
      opts.method || import_js_iden3_core40.DidMethod.Iden3,
      opts.blockchain || import_js_iden3_core40.Blockchain.Polygon,
      opts.networkId || import_js_iden3_core40.NetworkId.Amoy
    );
    const identifier = import_js_iden3_core40.Id.idGenesisFromIdenState(didType, currentState.bigInt());
    const did = import_js_iden3_core40.DID.parseFromId(identifier);
  ...
  async getGenesisDIDMetadata(did) {
    const identity = await this._storage.identity.getIdentity(did.string());
    if (identity) {
      return { nonce: 0, genesisDID: import_js_iden3_core40.DID.parse(identity.did) };
    }
    ...
    return { nonce: profile.nonce, genesisDID: import_js_iden3_core40.DID.parse(profile.genesisIdentifier) };
  }
  ...
  async sign(message, credential) {
    ...
    const signature = await this._kms.sign(keyKMSId, import_js_iden3_core40.BytesHelper.intToBytes(payload));
  ...
  async signChallenge(challenge, credential) {
    const keyKMSId = getKMSIdByAuthCredential(credential);
    const signature = await this._kms.sign(keyKMSId, import_js_iden3_core40.BytesHelper.intToBytes(challenge));
    return import_js_crypto12.Signature.newFromCompressed(signature);
  }
  ...
  async transitState(did, oldTreeState, isOldStateGenesis, ethSigner, prover) {
    ...
    const userId = import_js_iden3_core40.DID.idFromDID(did);
    let proof;
